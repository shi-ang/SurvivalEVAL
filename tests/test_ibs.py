...
def test_case_4_comparison_with_paper():
    ...
    train_right_limits[np.random.choice([True, False], n_samples, p=[0.25, 0.75])] = np.inf
    ...
    print(f"Average interval width: {np.mean(right_limits[np.isfinite(right_limits)] - left_limits[np.isfinite(right_limits)]):.2f}")
