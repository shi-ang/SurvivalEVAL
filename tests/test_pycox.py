...
def _logistic_hazard_loss(logits: torch.Tensor, idx: torch.Tensor, events: torch.Tensor) -> torch.Tensor:
    ...
    if event_mask.any():
        loss[event_mask] = -(sum_prev[event_mask] + log_hazard[event_mask, idx[event_mask]])
    if (~event_mask).any():
...
def _train_logistic_hazard(x_train, idx_train, events_train, cuts, epochs=40, batch_size=128):
    ...
    dataset = TensorDataset(
        torch.from_numpy(x_train), torch.from_numpy(idx_train), torch.from_numpy(events_train)
    )
...
def _predict_survival(model: nn.Module, x: np.ndarray, cuts: np.ndarray) -> pd.DataFrame:
...
def trained_pycox_evaluator():
    ...
    idx_train, events_train_disc = _make_discrete_targets(durations_train, events_train, cuts)
...
def test_pycox_concordance_and_brier(trained_pycox_evaluator):
    ...
    brier = evaluator.brier_score(target_time=float(np.median(durations_test)), IPCW_weighted=True)
