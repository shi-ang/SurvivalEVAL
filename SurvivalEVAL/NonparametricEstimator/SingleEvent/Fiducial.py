...
def _solve_qp_osqp(
    ...
    if result.info.status not in ["solved", "solved_inaccurate"]:
        ...
        warnings.warn(
            f"OSQP did not converge (status: {result.info.status}), falling back to scipy"
        )
...
def _solve_qp_scipy(
    Q: np.ndarray, q: np.ndarray, lb: np.ndarray, ub: np.ndarray
) -> np.ndarray:
    """
...
def fit_fiducial_interval_censor(
    ...
    if not np.all(np.isfinite(grid)):
        raise ValueError(
            f"Grid contains NaN/Inf. grid_low={grid_low}, grid_high={grid_high}"
        )
    ...
    Q, _ = _build_qp_matrices(
        ngrid, grid, lam
    )  # lambda parameter in LinInterpolation.R
...
def _demo():
    ...
    CI_valid = np.all(result["F_low"] <= result["F_median"]) and np.all(
        result["F_median"] <= result["F_high"]
    )
